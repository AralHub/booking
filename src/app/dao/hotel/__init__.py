...
#         # Получаем доступные комнаты с использованием логики из BookingDAO
#         available_rooms = await cls._get_available_rooms_using_booking_dao(
#             session,
#             hotel_ids,
#             check_in_date,
#             check_out_date,
...
#         amenities: list[int] = None,
#     ):
#         """Находит отели в указанном городе с учетом фильтров."""
#         # Находим город
#         db_city = await CityDAO.get_one_or_none(
#             session=session,
...
#         rooms_query = select(Room).where(Room.hotel_id.in_(hotel_ids))
...
#         # Получаем словарь с количеством забронированных комнат для каждого отеля
#         available_rooms = []
...
#             # Фильтруем комнаты для текущего отеля
...
#                 # Проверяем, что комната не полностью забронирована
#                 if booked_count < room_quantity:
#                     # Проверяем, что комната подходит по количеству гостей
#                     max_guests = room.max_guests
#                     if all(guest_count <= max_guests for guest_count in guests):
...
#         selected_rooms: dict,
#     ):
#         """Загружает полную информацию о подходящих отелях."""
...
#                 selectinload(cls.model.reviews),
#             )
#             .where(cls.model.id.in_(hotel_ids))
#         )
...
#                 "available_rooms": selected_rooms[hotel.id],
#             }

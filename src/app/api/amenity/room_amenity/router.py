...
async def get_room_amenity_category(
    category_id: int,
    session=SessionDep,
):
    """
    Получить категорию удобств комнат по ID
    """
    return await RoomAmenityCategoryDAO.get_one_or_none(
        session=session,
        filters=RoomAmenityFilter(id=category_id),
    )
